...
object RegistrationWithIdParser {
  ...
  implicit object RegistrationWithIdReads extends HttpReads[Either[Exception, ResponseWithId]] with Logging {

    override def read(method: String, url: String, response: HttpResponse): Either[Exception, ResponseWithId] =
      response.status match {
        case OK =>
          response.json.validate[ResponseWithId] match {
