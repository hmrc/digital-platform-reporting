...
class RecentSubmissionsController @Inject()(authAction: AuthAction,
  ...
  def get(operatorId: String): Action[AnyContent] = authAction(parse.default).async { implicit request =>
    recentSubmissionsRepository.findBy(request.userId, operatorId).map {
  ...
  def save(): Action[RecentSubmissionRequest] = authAction(parse.json[RecentSubmissionRequest]).async { implicit request =>
    val recentSubmissionDetails = RecentSubmissionDetails(request.userId, request.body.operatorId, clock.instant())
