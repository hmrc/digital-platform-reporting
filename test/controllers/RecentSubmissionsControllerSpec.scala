...
class RecentSubmissionsControllerSpec extends SpecBase
  ...
  ".get()" - {
    "must return recent submission when matching one is found" in {
      when(mockRecentSubmissionsRepository.findBy(userId)).thenReturn(Future.successful(Some(aRecentSubmissionDetails)))

      val request = FakeRequest(routes.RecentSubmissionsController.get())
    ...
    "must return NOT_FOUND when matching recent submission not found" in {
      when(mockRecentSubmissionsRepository.findBy(any())).thenReturn(Future.successful(None))
  ...
  ".save()" - {
    "must save pending enrolment and return OK" in {
      val recentSubmissionDetails = aRecentSubmissionDetails.copy(
        userId = userId,
        operatorDetails = Map(
          "operator-id-one" -> anOperatorSubmissionDetails.copy(operatorId = "operator-id-one"),
          "operator-id-two" -> anOperatorSubmissionDetails.copy(operatorId = "operator-id-two")
        ),
      ...
      val recentSubmissionRequest = RecentSubmissionRequest(
        operatorDetails = Map(
          "operator-id-one" -> anOperatorSubmissionDetails.copy(operatorId = "operator-id-one"),
          "operator-id-two" -> anOperatorSubmissionDetails.copy(operatorId = "operator-id-two")
        ),
